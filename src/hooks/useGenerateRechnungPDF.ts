...
export const useGenerateRechnungPDF = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (input: GenerateRechnungInput) => {
      const { data, error } = await supabase.functions.invoke('generate-rechnung-pdf', {
        body: input,
      });
    ...
    onSuccess: (data) => {
      // Download PDF
      downloadPDF(data.base64, `Rechnung_${data.rechnungsnummer}.pdf`);

      // Show success toast
      toast({
        title: 'Rechnung erstellt',
        description: `Rechnung Nr. ${data.rechnungsnummer} wurde erfolgreich erstellt und heruntergeladen.`,
      });
    ...
    onError: (error: Error) => {
      ...
      toast({
        title: 'Fehler',
        description: error.message || 'Fehler bei der PDF-Generierung. Bitte versuchen Sie es erneut.',
        variant: 'destructive',
      });
    },
  });
