...
export const useGenerateTreuhandvertragPDF = () => {
  const { toast } = useToast();

  return useMutation({
    mutationFn: async (input: GenerateTreuhandvertragInput) => {
      const { data, error } = await supabase.functions.invoke('generate-treuhandvertrag-pdf', {
        body: input,
      });
    ...
    onSuccess: (data) => {
      if (data.base64 && data.filename) {
        ...
        toast({
          title: 'Treuhandvertrag PDF erstellt',
