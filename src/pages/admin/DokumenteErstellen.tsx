...
const DokumenteErstellen = () => {
  ...
  useEffect(() => {
    // Guard: nur einmal pro bestellungId ausführen
    if (!bestellungId || processedBestellungRef.current === bestellungId) return;
    if (bestellungen.length === 0 || autos.length === 0) return;
    ...
    if (matchedAutos.length > 0) {
      ...
      // Markiere diese Bestellung als verarbeitet
      processedBestellungRef.current = bestellungId;
      ...
      // Auch bei Fehler markieren, um Endlosschleife zu verhindern
      processedBestellungRef.current = bestellungId;
