...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Generating Treuhandvertrag PDF with:', { kanzlei_id, kunde_id, bankkonto_id, insolvente_unternehmen_id, gender });
    ...
    console.log('Treuhandvertrag PDF generated successfully');

    const genderLabel = gender === 'M' ? 'Männlich' : 'Weiblich';
    const filename = `Treuhandvertrag ${genderLabel} ${sanitizeFilename(kunde.name)}.pdf`;
    ...
    console.error('Error in generate-treuhandvertrag-pdf function:', error);
    return new Response(
